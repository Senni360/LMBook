...
try {
  ...
  /* Source archives package beside their source directory. */
}
...
if (!releaseVersion.test(version))
  throw new Error(
    "A release needs a supported stable or numbered prerelease version.",
  );
const output = path.join(checkout, "release", version);
const args = process.argv.slice(2);
if (
  args.some((arg) =>
    /directories\.output|--config(?:=|$)|^-c(?:=|$)|--publish|^-p(?:=|$)/.test(
      arg,
    ),
  )
)
  throw new Error(
    "Desktop packages must stay local in the normal release/<version> folder.",
  );
