...
Owner clarification: someone with an existing Obsidian vault must be able to bring it into LMBook, generate summaries and other learning outputs, and edit/create files from LMBook that also appear in Obsidian. They propose embedding the exact app especially to retain plugins. This confirms bidirectional shared-file use as the desired outcome, not a one-time copied import. A connected vault folder is the proposed mechanism; it is not implemented. Plugin compatibility remains an explicit open question. Next discussion: which plugins or plugin-produced content must work inside LMBook, versus continuing to work in the separately installed Obsidian app?
