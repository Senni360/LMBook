...
**Publication instruction updated in the same session:** the owner authorized skipping remaining checks and publishing the existing 0.3.9–0.3.16 builds after checking for exposed credentials. This delivery uses the existing Windows installer and portable files, checks their embedded versions and matching application payloads, scans the extracted packages and tracked source for configured credentials/common secret patterns, and verifies GitHub's SHA-256 digests before publication. macOS downloads are not included in this batch. The 24-target native workflow below remains available, but was not used for this publication; do not describe these historical releases as freshly rebuilt on all platforms.
