# LMBook UX direction 06: Research-first

Standalone implementation prompt. Prepared 2026-09-17. Status: candidate for critique; not a selected production redesign.

**Organising choice:** Source comparison and a persistent working note.

**Protected signature — UX-06:** A claim shaped beside supporting and limiting evidence. Preserve this mechanism during critique; its detailed requirements and demonstration appear below.
...
Create a research desk for comparing material and producing a defensible piece of writing. The main working note stays available while sources change. The product should help the writer see what evidence does and does not support, without pretending AI can certify an argument.

Open the notebook to a source list, one reader and the current working note. Provide labelled swap/expand controls so left-handed or right-handed layout preferences do not become a new workflow. The note is not a tiny secondary scratchpad; it needs comfortable reading and editing space.

## Navigation, comparison and state

- Source search and filters distinguish imported source material, editable notes and requirement documents. Browsing a source does not automatically include it in an AI request.
- Source readers use closable tabs with path/title disambiguation. The working note is explicitly pinned as the writing target. Opening a different note offers Open for reference or Make working note so research navigation cannot silently redirect edits.
- Compare two sources through an obvious action. The layout has a clear return to one reader. Do not show a fourth narrow column: the source area can split while the working note is temporarily collapsed, or use an adjustable source-over-note arrangement when width requires it.
- Quoting has a visible Insert quotation action plus drag-and-drop. Insert into an explicit location with citation metadata and preserve source wording. A quotation inserted into an unsaved note remains an unsaved edit, not a completed external file write.
- Resize reader/note widths and stacked comparison heights. Source positions are independent; no automatic scroll coupling across different documents. Narrow layouts offer Source A / Source B / Working note with named return context.

## Placement of features and routine workflows

The working-note toolbar offers Ask about evidence, Review argument and Create learning material. Notebook-level Practice and Listen remain in the main navigation, with saved artifacts accessible immediately. A small collection selector gathers sources for a question or draft but does not move their files.

Import and vault connection originate in the source list with a new-notebook option. Notes remain editable through the same conflict-safe editor. Source cleanup retains originals. Export options distinguish the current note, the vault and a complete backup. Provider/model controls remain in Settings.

## AI suggestions, edits and background work

AI findings appear as specific questions about the selected claim: Does this source cover that population? Does it establish a cause? Is this a competing definition? Avoid a verdict badge for the entire essay. One review panel shows the claim, examined excerpts, limitations and optional proposed revision.

Keep suggested links and organisational changes in a grouped notebook queue rather than beside every source sentence. A proposed writing change must not overwrite the working note on arrival. Automatic changes remain inspectable under the selected control mode, with revision checks and Undo. Background retrieval/checking runs appear in a compact activity drawer; partial results state their actual coverage.

## PROTECTED UNCONVENTIONAL IDEA — UX-06: the claim workbench

**Core requirement:** let the writer construct or narrow a claim while directly arranging the evidence that supports it and the evidence that limits it.

From a selected sentence or New claim open a bounded workbench with an editable claim, **Supports this**, **Limits or qualifies this**, and **Still unresolved** areas. Users can place exact source excerpts into these roles through labelled buttons as well as dragging. AI may propose a role, but the user can inspect and change it. The areas are working classifications, not proof levels or a requirement to manufacture opposing evidence.

Changing the claim keeps its evidence visible and marks prior AI judgments as needing another look. An empty limitation area means none recorded, not no limitations exist. Permit a defensible conclusion supported by one source and an explicitly unresolved question; never reward filling every area. No argument-strength score.

Keep the workbench separate from the live note until the writer chooses Insert or Replace selected claim. Show where the text will go and which citations will accompany it. Preserve the broader evidence arrangement as optional local research notes, not hidden modifications to the vault. If source revisions change, retain examined excerpts and label them historical. A narrow window presents the claim above sequential evidence sections.

**Protected acceptance:** the claim is editable beside evidence roles, evidence assignments are inspectable/changeable and committing to the note is explicit. A conventional AI feedback list cannot replace this mechanism.

**Demonstration:** start with the overbroad relocation claim. Put the pilot survey under support, the unknown reasons under limitations, and narrow the claim to what the material establishes. Then insert it with citations into the working note. Repeat with no counterevidence and confirm the UI does not invent a required opposing side.

## Conflict checks and tradeoff to evaluate

The workbench is for consequential claims, not a form every sentence must pass through. It should open from current work and return cleanly. Check that the source comparison layout and workbench are separate states of one area rather than accumulating panels. This candidate should excel for a PWS or essay; it fails if basic reading and ordinary flashcard practice require navigating a research apparatus first.
