# LMBook UX direction 07: Outline-led notebook

Standalone implementation prompt. Prepared 2026-09-17. Status: candidate for critique; not a selected production redesign.

**Organising choice:** Topics containing sources, notes and learning material.

**Protected signature — UX-07:** A reversible question-based lens over the topic outline. Preserve this mechanism during critique; its detailed requirements and demonstration appear below.
...
Organise the notebook around its subject matter. A topic can bring together several notes, source passages, questions, decks and episodes without duplicating or moving the underlying files. The user should think about their course or project rather than the storage format of each object.

Open to a readable outline and a selected topic's main note. A small related-material list shows only what actually exists. Keep a direct Files view for existing-vault users and a topic search that also returns unassigned items. Empty notebooks can begin with a note; requiring an elaborate outline first would undermine the direction.

## Navigation, topics and state

- The primary left navigation is a collapsible topic outline. Top-level entries have clear names and restrained disclosure controls. Support nested topics but avoid forcing every note into a deep hierarchy.
- The centre shows the selected topic's note or material. A compact local switcher opens its sources, questions and learning artifacts. Use a recent-items menu instead of an extra global strip of tabs. Back restores both topic and reading position.
- The same source may support several topics. Show that as a relationship to one object, not separate files. Renaming a topic does not rename a shared Markdown file; file operations remain explicit in Files.
- Reordering topics has keyboard Move up/down/into controls alongside dragging. Moving an object between groups should state whether it changes only organisation or the actual filesystem. Never infer destructive file actions from visual arrangement.
- Resize outline/main widths and optionally a related-material panel. Long outlines scroll independently from content. Narrow layouts use a searchable topic chooser with breadcrumbs. Restoring a deleted topic restores its references without recreating deleted source files silently.

## Placement of features and routine workflows

A topic offers Read, Ask, Practice and Listen when meaningful, with Create actions in empty states. Notebook-wide activity entry points remain available for material spanning several topics. Source selection states whether the user chose specific items, a topic's current sources or a broader set. An outline rearrangement cannot silently change the evidence of an already generated answer.

Keep source requirements separate from personal topic headings. A topic can have a linked learning goal, but its content count does not represent progress or mastery. Decks and audio retain their original source snapshots. Import offers a new/existing notebook and optional later topic assignment; unassigned material is a visible collection rather than hidden or automatically discarded.

## AI suggestions, edits and background work

Place topic-specific suggestions near their topic's Review action and collect everything in a notebook review view. Suggested grouping, split topics and contextual term distinctions show their reasons and affected objects. Proposed outline changes use a before/after tree with individual decisions; rejecting one change must not force rejection of an unrelated batch.

AI-generated organisation remains a proposal unless the selected automation mode allows it and the implementation supports safe recovery. Distinguish missing source coverage, unresolved questions and actual learning performance. Background activity uses a quiet notebook status control; errors name affected topics and underlying files. Applied changes retain a history with reversible organisation actions separate from file edits.

## PROTECTED UNCONVENTIONAL IDEA — UX-07: a question lens

**Core requirement:** the user can temporarily navigate the same notebook through their questions instead of its topic hierarchy, without reorganising or duplicating the actual content.

Offer a visible **View by questions** control beside the outline title. In this mode, explicit user questions become navigational entries, each linked to relevant topics and material. Example entries: Who was represented in the survey? What can this evidence explain? Which definitions differ between sources? Build the demonstration from authored fixture mappings; do not pretend the model has automatically answered those questions.

A question can draw from multiple topics. Opening it shows the relevant passages, the user's unfinished notes and links back to their original topics. A missing connection is labelled No material linked yet, not Unanswerable. Suggested associations are inspectable and removable. Users can add a question directly or save one from a conversation. Do not force an AI generation step just to navigate.

Switching back restores the previous outline expansion and position. The question view has its own remembered selection. Editing a note from either view updates the same underlying object. Adding or removing a question relationship does not move files, change requirements or alter a saved deck. In narrow layouts the same view switch remains available with the current question named.

**Protected acceptance:** questions genuinely change navigation, relationships can cross topic boundaries, and the original outline is untouched and recoverable. A list of suggested chat prompts below the outline is insufficient.

**Demonstration:** start in the survey-methods topic, switch to the question about causality, open evidence from methods and relocation topics, edit the same note, and return to the original outline position. Repeat with an unanswered question and a renamed source.

## Conflict checks and tradeoff to evaluate

Prevent Topics, Files and Questions from becoming three competing mandatory taxonomies. Topics is the default; Files preserves familiar access; Questions is an explicit alternate lens. Keep their labels stable and explain the view switch once in place. This candidate should clarify a structured course or project, but evaluate the work required to maintain the outline and the discoverability of unassigned material.
