# LMBook UX direction 03: Notebook tabs

Standalone implementation prompt. Prepared 2026-09-17. Status: candidate for critique; not a selected production redesign.

**Organising choice:** Notes / Ask / Practice / Listen destinations.

**Protected signature — UX-03:** An explicit context handoff between activities. Preserve this mechanism during critique; its detailed requirements and demonstration appear below.
...
Create a notebook with four stable, clearly labelled activity destinations: Notes, Ask, Practice and Listen. Each gets an appropriate layout, while the notebook identity, source controls and interaction language remain familiar. This should be easy to explain to a newcomer without forcing all tasks through a wizard.

Open Notes for a fresh notebook, with Create note and Add material. Restore the last activity for a returning notebook. Keep all four destinations visible even before material exists; empty states explain the next useful action instead of hiding capabilities.

## Navigation, local tabs and state

- The notebook switcher and four destinations form one consistent header or rail. Do not duplicate them across multiple navigation bars.
- Notes has a file tree and closable document tabs with preview/pin behaviour. Ask has a conversation list and one active conversation. Practice has a deck library and a focused session. Listen has an episode library and a player. Avoid a second general-purpose tab strip inside every activity.
- Switching activities preserves note drafts, chat input, source selection, deck direction and playback. A playing episode continues with a compact shared transport; typing practice pauses its active interaction when hidden rather than accepting accidental keystrokes.
- Document tab Close closes a view. Exiting practice retains progress and returns to its deck. Returning from episode settings restores playback context. These actions need specific labels rather than one ambiguous X everywhere.
- Each activity may have its own remembered split sizes. Notes supports horizontal file/reader resizing; Ask supports conversation/evidence resizing; Listen supports player/transcript height adjustment. A visible Reset layout applies to the current activity. Narrow windows retain the four labels through a compact accessible navigation control.

## Placement of features and routine workflows

Notes owns file creation, import and source browsing. Sources remain reachable from each activity through its scope control. Ask owns new conversations and citation review. Practice opens saved decks before creation controls and clearly separates pair editing from studying. Listen opens saved episodes before generation settings. Notebook export, backup and Connections live in a consistent menu outside the activity-specific toolbar.

The selected-sources control is visually consistent but scoped to the actual task; do not assume a deck and a conversation must share one mutable global selection. Show when an activity reuses a prior choice. Earlier answers and episodes retain their historical evidence.

## AI suggestions, edits and background work

Review suggestions beside their relevant activity: note links in Notes, claim evidence in Ask, deck issues in Practice, script findings in Listen. A shared Review entry provides all pending decisions, deduplicated by identity. Use one reusable change-comparison panel across these contexts.

A suggestion linking two activities should name both and offer a deliberate navigation action, not automatically switch screens. Background jobs use a shared activity popover with grouped progress. Errors appear beside the failed task as well. An automatic edit creates an activity record and an Undo route without opening a review modal on every application.

## PROTECTED UNCONVENTIONAL IDEA — UX-03: carry context across activities

**Core requirement:** provide a small explicit handoff strip that carries a chosen passage or question across Notes, Ask, Practice and Listen without copying it into unrelated files or silently starting generation.

From selected text or a paragraph's menu choose **Use in another activity**. A compact strip names the passage, source and revision and offers the four valid destinations. The user can choose a destination now or switch normally while retaining the strip. Only one active handoff occupies the strip; replacing it offers keep/cancel rather than quietly discarding a prepared task.

On arrival, show the proposed use: ask about this passage, prepare practice from it, or include it in an audio brief. The destination must confirm whether this passage replaces or supplements its current source scope. A carried passage does not rewrite an existing deck, alter an old episode or run a paid job. The user can inspect, remove or apply the context before proceeding.

The strip collapses to a labelled summary after arrival and disappears when consumed or dismissed. Retain the source anchor and user question through the handoff. If the source is missing or changed, show a historical excerpt with a recovery choice. At narrow widths use a compact single-row entry that expands; do not obstruct the composer or audio controls.

**Protected acceptance:** context survives a destination change, its source remains visible, and using it requires a clear destination action. A clipboard button or automatic global source selection is not an equivalent.

**Demonstration:** carry the survey passage from Notes into Ask, add a question, then prepare a Practice draft without regenerating the existing German deck. Dismiss the handoff and verify both destinations retain unrelated work. Repeat with a stale source and a destination that already has selected material.

## Conflict checks and tradeoff to evaluate

The four destinations risk becoming silos. Test the handoff and direct links without flattening every activity into one giant workspace. Keep the temporary strip visually subordinate so it does not become a fifth destination. This candidate should make feature discovery easy; it loses if routine cross-reference work requires excessive switching or users cannot tell which scope belongs to which task.
