...
**0.3.4 adds coordinated interface motion** throughout LMBook: travelling selection markers, source and citation reveals, responsive lists, single-face flashcards, playback and save feedback. Reduce motion in Connections & settings → Your library → Motion, or use your device's preference. Quit an older running version before opening LMBook. Desktop is the primary delivery target; the browser preview uses a separate library. See [0.3.4 release notes](docs/releases/0.3.4.md) and the [LMBook rebrand](docs/releases/0.3.3.md).
