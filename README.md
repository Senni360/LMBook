...
**0.3.7 refines Ink interactions:** coordinated pen contours for buttons and fields, one chat writing surface, consistent heading highlights and clearer task labels. Local installer, portable and interaction video are under `release/0.3.7/`; this build is not yet a published GitHub release. Reduce motion in Connections & settings → Your library → Motion, or use your device's preference. Quit an older running version before opening LMBook. Desktop is the primary delivery target; the browser preview uses a separate library. See [0.3.7 release notes](docs/releases/0.3.7.md), [research and evaluation](docs/ui/ink-interactions.md), and the [LMBook rebrand](docs/releases/0.3.3.md).
